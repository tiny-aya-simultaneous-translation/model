...
def main() -> None:
    ...
    p.add_argument("--audio_samples", type=int, default=4, help="AR wavs to decode+write")
    ...
    for j, idx in enumerate(order):
        ...
        if j < args.audio_samples:
            ...
            wav = mimi.decode(undo_codebook_delay(gen)).numpy()
            import soundfile as sf

            sf.write(os.path.join(args.output_dir, f"ar_sample_{j}.wav"), wav, 24000)
    ...
    if args.wandb_run:
        ...
        # eval/* is define_metric'd against global_step in the trainer, so
        # these points land on the training x-axis.
        run.log(metrics)
        run.finish()
        print(f"[proxy] backfilled to {args.wandb_run} @ step {step}", flush=True)
